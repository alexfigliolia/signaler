...
io.on('connection', (socket) => {
  ...
  socket.on('offer', (offer) => {
    if(offer.to in clients) {
      ...
      io.to(clients[offer.to].id).emit('offer', {offer: offer.offer, from: offer.from, image: image});
    } else {
      io.to(socket.id).emit('friendConnectionError', 'There was a connection error. Your friend may be unavailable or in another vidchat');
    }
  ...
  socket.on('remoteStream', (stream) => {
    if(socket.id in clients) io.to(socket.id).emit('remoteStream', stream);
  });

  socket.on('endChat', (otherGuy) => {
    if(otherGuy in clients) io.to(clients[otherGuy].id).emit('endChat', 'end the chat');
  });

  socket.on('busy', (otherGuy) => {
    if(otherGuy in clients) io.to(clients[otherGuy].id).emit('busy', 'This user is in another chat');
  })
